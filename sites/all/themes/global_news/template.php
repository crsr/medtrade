 ...
 */
function global_news_html_head_alter(&$head_elements) {
 ...
 */
function global_news_preprocess_search_block_form(&$vars) {
  $vars['search_form'] = str_replace('type="text"', 'type="search"', $vars['search_form']);
}

function global_news_form_alter(&$form, &$form_state, $form_id) {
 ...
 */
function global_news_preprocess(&$vars, $hook) {
  $vars['global_news_path'] = base_path() . path_to_theme();
}
...
function global_news_preprocess_html(&$vars) {
  ...
  $fixed_layout = theme_get_setting('global_news_layout');  
  ...
  if (!$vars['is_front']) {
    ...
    if (isset($page_name)) {
      $vars['classes_array'][] = global_news_id_safe('page-' . $page_name);
    }

    $vars['classes_array'][] = global_news_id_safe('section-' . $section);
 ...
 */
function global_news_preprocess_page(&$vars) {
 ...
 */
function global_news_preprocess_maintenance_page(&$vars) {
  ...
  drupal_add_css(drupal_get_path('theme', 'global_news') . '/css/maintenance-page.css');
}
 ...
 */
function global_news_preprocess_node(&$vars) {
  // Striping class
 ...
 */
function global_news_preprocess_block(&$vars, $hook) {
 ...
 */
function global_news_menu_tree($vars) {
 ...
 */
function global_news_field__taxonomy_term_reference($vars) {
  $output = '';
 ...
 */
function global_news_breadcrumb($vars) {
  
  $breadcrumb = isset($vars['breadcrumb']) ? $vars['breadcrumb'] : array();

// Determine if we are to display the breadcrumb.
  $show_breadcrumb = theme_get_setting('global_news_breadcrumb_display');
  if ($show_breadcrumb == 'yes') {
  
	  if (theme_get_setting('global_news_breadcrumb_hideonlyfront')) {
		$condition = count($breadcrumb) > 1;
	  } else {
		$condition = !empty($breadcrumb);
	  }
	  
	  if(theme_get_setting('global_news_breadcrumb_showtitle')) {
	  ...
	  $separator = theme_get_setting('global_news_breadcrumb_separator');
 ...
 * @return bool
 */
function global_news_tabs_float() {
  $float = (bool) theme_get_setting('global_news_tabs_float');
  $float_node = (bool) theme_get_setting('global_news_tabs_node');
...
function global_news_id_safe($string) {
  // Strip accents
 ...
 * @param array $vars
 */
function _global_news_rdf($vars) {
  $rdf = new stdClass();
 ...
 * @ingroup themeable
 */
function global_news_menu_link(array $vars) {
  ...
  $element['#attributes']['class'][] = global_news_id_safe($element['#title']);
 ...
 */
function global_news_preprocess_menu_local_task(&$vars) {
 ...
 */
function global_news_menu_local_tasks(&$vars) {
  $output = '';
...
function global_news_feed_icon($variables) {
  $text = t('Subscribe to !feed-title', array('!feed-title' => $variables['title']));
  
  if ($image = theme('image', array('path' => drupal_get_path('theme', 'global_news').'/images/feed.png', 'width' => 12, 'height' => 12, 'alt' => $text), t('Syndicate content'))) {
    return '<a href="'. $variables['url'] .'" class="feed-icon">RSS Feed'. $image. '</a>';
  }
}

function global_news_bootstrap_modal($variables) {
  $body = $variables['body'];
...
function global_news_styling(){
  $_path = drupal_get_path('theme', 'global_news');
 
  $theme_col = theme_get_setting('global_news_theme_styling_color');
  
  include_once ($_path.'/css/style_'.$theme_col.'.css');
}

function header_date(){
  $date_output = theme_get_setting('global_news_date_output');
  if ($date_output == 1) {
    echo date('l, F j, Y \L\a\s\t \U\p\d\a\t\e: g:i A');
  }
}